...
class FrozenShoulder:
    def __init__(self, model_path, n_time_steps=60, step_size=45, ROM_left_flexion=90):
        ...
        # Exeercise ROM thresholds
        self.ROM_left_flexion = ROM_left_flexion
...
if "threshold" not in st.session_state:
    st.session_state.threshold = 90  # Initialize threshold in session state
...
if app_mode == "Settings":
    ...
    st.session_state.threshold = epi2.slider("**SHOULDER FLEXION LEFT [ANGLE THRESHOLD]**", min_value=70, max_value=120, value=st.session_state.threshold)
    ...
    with c1:
        ...
        ## Dashboard
        detector = FrozenShoulder("20250106v1_45stepsize.h5", ROM_left_flexion=st.session_state.threshold)
        ...
        while vid.isOpened():
            ...
            if len(lmList) != 0:
                ...
                current_state = detector.get_state(degree_of_movement, st.session_state.threshold)
