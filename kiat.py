...
if app_mode == "Video":
    ...
    while vid.isOpened():
        ...
        imgRGB = cv2.resize(img, (0, 0), fx=0.8, fy=0.8)
...
if app_mode == "Settings":
    ...
    tab1, tab2, tab3 = st.tabs(["**Tab 1 / 3**", "**Tab 2 / 3**", "**Tab 3 / 3**"])

    with tab1:
        st.header("Tab 1 / 3")

        api1, api2, api3, api4 = st.columns(4)
        api1.slider("**ARMPIT LEFT**", value=5, min_value=1, max_value=30)
        st.divider()
        api2.slider("ARMPIT RIGHT", value=5, min_value=1, max_value=30)
        st.divider()
        api3.slider("CIRCLE LEFT", value=5, min_value=1, max_value=30)
        st.divider()
        api4.slider("CIRCLE RIGHT", value=5, min_value=1, max_value=30)
    with tab2:
        st.header("Tab 2 / 3")
